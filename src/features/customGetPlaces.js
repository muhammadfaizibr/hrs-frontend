const customGetPlaces = async (data) => {
    
    let url = 'http://127.0.0.1:8000/api/';
    if (data.filters?.sortBy === "recommendations"){
        url = url+`recommendations?query=${data.query ?encodeURIComponent(data.query) : ''}&city=${data.filters?.city ? data.filters?.city.toLocaleLowerCase() : ''}&place_type=${data.filters?.placeType ? data.filters?.placeType : ''}&subcategories=${data.filters?.subcategories ? data.filters?.subcategories : ''}&amenities=${data.filters?.amenities ? data.filters?.amenities : ''}`
    }

    else{
        url = url+`place-list-create?query=${data.query ?encodeURIComponent(data.query) : ''}&page=${data.page ? data.page : ''}&min_rating=${data.filters?.rating ? data.filters?.rating : ''}&city=${data.filters?.city ? data.filters?.city.toLocaleLowerCase() : ''}&place_type=${data.filters?.placeType ? data.filters?.placeType : ''}&subcategories=${data.filters?.subcategories ? data.filters?.subcategories : ''}&number_of_reviews=${data.filters?.noOfReviews ? data.filters?.noOfReviews : ''}&combined_amenities=${data.filters?.amenities ? data.filters?.amenities : ''}`
    }

    try {
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
        });

        if (!response.ok) {
            throw new Error(`Error: ${response.statusText}`);
        }

        const result = await response.json();
        return result;
    } catch (error) {
        console.error("Fetch error:", error.message);
        return { error: error.message };
    }
};

export default customGetPlaces;